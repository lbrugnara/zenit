...
ZnesArraySymbol* znes_symbol_array_new(const char *name, ZirArrayType *zir_array_type, ZnesSegment segment, uint16_t address)
{
    ...
    for (size_t i=0; i < zir_array_type->length; i++)
    {
        ...
        ZnesSymbol *element = znes_symbol_new(buf, zir_array_type->member_type, segment, address + (member_size * i));
...
void znes_symbol_array_free(ZnesArraySymbol *array_symbol)
{
    ...
    fl_array_free_each_pointer(array_symbol->elements, (FlArrayFreeElementFunc) znes_symbol_free);
