...
void zenit_test_lexer_run(struct ZenitLexerTest *tests, size_t count)
{
    for (size_t i=0; i < count; i++)
    {
        ...
        flut_expect_compat("Tokenization must return a valid array of tokens", tokens != NULL);

        for (size_t j=0; test.tokens[j] != ZENIT_TOKEN_EOF; j++)
            flut_vexpect_compat(test.tokens[j] == tokens[j].type, "\"%s\": expected token at position %zu to be %s", test.source, j, zenit_token_print(tokens[j].type));
