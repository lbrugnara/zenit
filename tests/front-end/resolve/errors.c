...
void cenit_test_resolve_errors(void)
{
    ...
    bool valid_parse = cenit_parse_string(&ctx, source);
    bool valid_resolve = valid_parse ? cenit_resolve_symbols(&ctx) : false;

    fl_expect("Resolve pass must fail with 1 errors", !valid_resolve && cenit_context_error_count(&ctx) == 1);
    fl_vexpect(ctx.errors[0].type == CENIT_ERROR_DUPLICATED_SYMBOL, 
        "Error must be a duplicated symbol (<source>:%u:%u): %s", ctx.errors[0].line, ctx.errors[0].col, ctx.errors[0].message);
