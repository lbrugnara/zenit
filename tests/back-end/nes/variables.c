...
void zenit_test_nes_literal_variables(void)
{
    ...
    fl_expect("Data segment at 0x15 should be 0x0B (arrarr_ref lo)",    nes_program->data.bytes[0x15] == 0x0B);
    fl_expect("Data segment at 0x16 should be 0x80 (arrarr_ref hi)",    nes_program->data.bytes[0x16] == 0x80);
